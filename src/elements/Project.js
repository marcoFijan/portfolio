...
export default function ProjectSection({
  ...
  return (
    <Link
      className="bg-gradient-to-tr from-color-bg-top/[0.2] to-color-bg-bottom/[0.3] rounded-2xl p-5 border border-color-border block group"
      href={projectLink || "#"}
      title={`Bekijk project: ${projectName}`}
    >
      <section className="rounded-2xl border-color-border group-hover:border-white/[0.8] transition-all duration-500 ease-in-out  border overflow-hidden  w-full h-full px-10 lg:px-20 pt-5 lg:pt-10 pb-0 relative">
        <article className=" flex justify-between items-start gap-8 relative text-white fill-white stroke-color-bg-top">
          <section>
          ...
          <svg
            viewBox="0 0 110 110"
            className="fill-current stroke-inherit shrink-0 w-10 h-10 mt-5 rotate-[135deg] group-hover:rotate-[540deg] transition-all duration-500"
          >
        ...
        <div className="group-hover:translate-y-4 translate-y-10 transition-all duration-500 aspect-video w-full lg:w-4/5 mx-auto overflow-hidden rounded-t-2xl bg-color-bg-bottom border border-color-border pt-1">
          <SvgBrowserToolbar />
