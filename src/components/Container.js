...
export default function Container({
  ...
  const classes = [
    ...
    grid
      ? "grid grid-cols-12 gap-x-4 gap-y-4 sm:gap-x-8 lg:gap-x-12 xl:gap-x-24"
      : "",
    className,
  ]
    .filter(Boolean)
  ...
  if (bgClass) {
    return (
      <section className={`${bgClass} py-32 dark relative`}>
        <Wave
          className="-translate-y-[99%] absolute top-0"
          colorClass={bgClass.replace("bg-", "text-")}
          startAt={15}
        />
        {content}
        <Wave
          className="translate-y-[99%] absolute bottom-0"
          colorClass={bgClass.replace("bg-", "text-")}
          side="left"
          startAt={75}
        />
